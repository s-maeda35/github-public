...
def foecast_info():
    ...
    for spotCounter in range(len(load_urls)):  #福岡～佐賀までループさせる
        for i in range(len(day_type)):  #当日と翌日の気象情報を取得
            html=requests.get(load_urls[spotCounter])
            soup=BeautifulSoup(html.content,'html.parser')
            dayType=soup.find(id=day_type[i])
            for j in range(len(in_item)):  #気温、降水確率、降水量、湿度、風速を取得
                ...
                toDay.append(day_result)
    df = pd.DataFrame(toDay,columns
    ...
    #print(df)
    #df.to_csv('forecast.csv',encoding='shift_jis')
    return df
